...
-(void) initData
{
    ...
    NSArray *temp = [[self readFile] componentsSeparatedByString:@":"];
...
//    self.navigationItem.rightBarButtonItem = _homeButton;
    ...
    self.title = @"FreedomsWeather";
    
    _sendNowButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    ...
    _sendSevenButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    ...
    _sendTodayButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
...
-(void)sendRequest:(NSURLRequest *)request
{
    ...
    AFJSONRequestOperation *jsonRequest = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        _weatherInfoKeys = [weatherInfo allKeys];
...
-(void) nowAction
{
    ...
    if (cityCode) {
        ...
        self.title = [[_cityTextfield text] stringByAppendingString:@"实时天气"];
    }else{
...
-(void) sevenAction
{
    ...
    if (cityCode) {
        ...
        NSURLRequest *request = [[NSURLRequest alloc] initWithURL:url];
        [self sendRequest:request];
        self.title = [[_cityTextfield text] stringByAppendingString:@"天气7天预报"];
    }else{
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Sorry" message:@"请输入正确的地区名称" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alertView show];
    }
...
-(void) todayAction
{
    ...
    if (cityCode) {
        ...
        NSURLRequest *request = [[NSURLRequest alloc] initWithURL:url];
        [self sendRequest:request];
        self.title = [[_cityTextfield text] stringByAppendingString:@"今日天气"];
    }else{
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Sorry" message:@"请输入正确的地区名称" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alertView show];
    }
